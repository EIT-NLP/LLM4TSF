...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, **kwargs):
        batch_x = inputs[0]  # [B, seq_len]
        batch_y = inputs[1]  # [B, total_len]

        #  [B, L] -> [B, L, 1]
        batch_x = batch_x.unsqueeze(-1)
        batch_y = batch_y.unsqueeze(-1)

        outputs = model(batch_x)  # [B, pred_len, 1] or [B, total_len, 1]

        # Compute loss only on the prediction horizon
        loss_fn = torch.nn.MSELoss()
        loss = loss_fn(outputs, batch_y[:, -self.args.pred_len:, :])
        return (loss, outputs) if return_outputs else loss

    def get_train_dataloader(self):
        """Override to use custom collate_fn and DataLoader settings."""
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=None):
        ...
        with torch.no_grad():
            ...
            # Extract only the prediction horizon from labels
            labels = batch_y[:, -self.args.pred_len:, :]
        return None, outputs, labels


def model_train(dataset_tr, dataset_va, dataset_te, test_metrics_path, args):
    ...
    training_args = TrainingArguments(
        ...
        push_to_hub=False,
        bf16=True,  # Use bfloat16 for faster training on supported hardware
    ...
    # Save test metrics (only on main process in distributed training)
    if trainer.accelerator.is_main_process:
        with open(test_metrics_path, "a") as f:
