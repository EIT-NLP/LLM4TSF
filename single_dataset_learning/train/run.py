...
parser.add_argument('--test_metrics_path', type=str, default='main/single_dataset_learning/train')
parser.add_argument('--outdir_base', type=str,default='main/single_dataset_learning/train')
args = parser.parse_args()
def run():
    name_list = ["ETTh1","ETTh2","ETTm1","ETTm2","electricity","exchange_rate","Solar","weather"]
    for name in name_list:
        # To accelerate the reading of large-scale data and reduce GPU memory usage, the Arrow format is adopted.
        ...
        if accelerator.is_main_process:
            with open(args.test_metrics_path, "a") as f:
                f.write(f"{name}\n")
        model_train(dataset_tr,dataset_va,dataset_te,args.test_metrics_path,args)
